...
public class PinyinUtil {
    ...
    protected PinyinUtil(boolean hasChinaCollator) {
        ...
        synchronized (PinyinUtil.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    ...
                    sInstance = new PinyinUtil(true);
                    return sInstance;
                }
            }
            Log.w(TAG,
                    "There is no Chinese collator, PinyinUtil is disabled");
            sInstance = new PinyinUtil(false);
            return sInstance;
        }
    }
